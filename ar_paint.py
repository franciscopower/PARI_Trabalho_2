...
def keyboardMapping(k, I, I_f, frame, AR, brush_size, opacity, clr, show_tool):
    ...
    # Save image
        ...
        if save_state:
            ...
            print("\nOur algorithm found this artwork very interesting...\nWe rate it ")
            print(score_str[0:score] + "\n(out of 5 stars)")
            print("You could share it on instagram, but we'll sell it... ;)\n(Special thanks to Mark Zuckerberg)")
        else:
...
def paint(drawing, frame, I, p1, p2, color, brush_size, AR, opacity):    
    ...
    # Paste blank canvas drawing on video frame
    if AR:
        if opacity == 1:
            I_f = np.copy(frame)
            hsv = np.copy(I)
...
def main():
    ...
    try:
        ...
        print(args.json_file + " file does not exist, is empty, or does not contain the right information." + Fore.RESET)
        ...
        if not ( "limits" in limits_dict.keys() and "color_mode" in limits_dict.keys() ):
            print(Fore.RED + "-----------------ERROR-----------------")
            print(args.json_file + " file does not contain the correct information." + Fore.RESET)
    ...
    t : toggle drawing tool marker \n 
    w, s : write/save drawing to file (if it's any good)\n"""
    ...
    print("\nThank you for using AR Paint and making the world a bit more colorful\n")
